 ...
 */
export function createStorage(namespace = '', initState = {}, type = STORAGE_TYPES.LOCALSTORAGE, maxAge = 0) {
  if (typeof namespace !== 'string' || namespace.trim().length === 0) {
    throw new Error('命名空间错误')
  }

  if (type !== STORAGE_TYPES.LOCALSTORAGE && type !== STORAGE_TYPES.SESSIONSTORAGE) {
    throw new Error('storage 类型错误')
  }

  // 是否设置有效时间(如果是 sessionStorage 则忽略)
  const hasMaxAge = type === STORAGE_TYPES.LOCALSTORAGE && typeof maxAge === 'number' && maxAge > 0
  ...
  const str = window[type].getItem(namespace)
  const prevState = decode(str)
  window[type].setItem(namespace, encode({ ...initState, ...prevState }))
